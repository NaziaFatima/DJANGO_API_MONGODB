...
class UserProfileViewSet(ModelViewSet):
    serializer_class = MongoUserResponseSerializer

    def get_queryset(self):
        return MongoUser.objects.all()

    @action(detail=True, methods=['patch'])  # , url_path=r'update_profile',)
    def update_profile(self, request, *args, **kwargs):
        instance = self.get_object()
        serializer = MongoUserResponseSerializer(
            instance, data=request.data, partial=True)
        serializer.is_valid(raise_exception=True)
        if (
                serializer.validated_data['last_name'] and
                serializer.validated_data['dob'] and
                serializer.validated_data['gender'] and
                serializer.validated_data['country']  # and not
                # serializer.validated_data['profile_updated']
        ):
